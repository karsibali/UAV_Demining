...
def environment(sz):
    ...
    composition = np.random.randint(5, size=sz)
    ...
    env = np.column_stack((moisture, composition, uniformity, magnetic, vegetation, weather, illumination))
    np.random.shuffle(env)
    return env


if __name__ == '__main__':
    env = environment(16000)
    np.savetxt('env_db.txt', env, fmt='%i', delimiter=' ')