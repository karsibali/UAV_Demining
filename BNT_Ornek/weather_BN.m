...
clc;clear;
%% Create the graphical model and the Bayesian Network
N = 4; 
dag = zeros(N,N);
C = 1; S = 2; R = 3; W = 4;
dag(C,[R S]) = 1;
...
bnet2 = mk_bnet(dag, ns, 'names', {'cloudy','S','R','W'}, 'discrete', 1:4);
seed = 0;
rand('state', seed);
...
load('weather-samples')
% Loaded data was 0 for False and 1 for True
% This causes problem in BNT so we change it to 1: False, 2: True
% We change the type to double since learn_params does not work with int64
samples = double(samples + 1); 
ncases = size(samples, 1);
cases = cell(4, ncases);
cases(:,:) = num2cell(samples');

% Now learn parameters from samples
bnet2 = learn_params(bnet2, cases);
